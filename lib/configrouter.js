...
class ConfigRouter {
      ...
      for (let route of currentRouteList) {
        ...
        if (this.router.has(route) === true) {
          ...
          if (isArray(prevRoute) === true) {
            /*
            for (let r of prevRoute) {
              if (r.method === newRoute.method) {
                throw new Error(`Duplicate route: '${route}' under method '${r.method}'`);
              }
            }
            */
            ...
            /*
            if (prevRoute.method === newRoute.method) {
              throw new Error(`Duplicate route: '${newRoute.route}' under method '${newRoute.method}'`);
            }
            */
  ...
  route(req, res) {
    ...
    if (matchResult !== undefined) {
      ...
      this.routeFinder(req, res, undefined, "/notFound");
    }
  }
  ...
  checkRequiredRoutes(method, route) {
    if (route["/notFound"] === undefined) {
      throw new Error(
        `'${method}' method has not defined the required '/notFound' route`
      );
    }
  }
  ...
  sanitizeRoutes(method, route) {
    ...
    if (route.search('^/') !== 0) 
      throw new Error(`Route '${route}' defined under method ${method} must start with '/'`);
  }
}
